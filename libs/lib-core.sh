...
if [[ ! "${REPO_DIR}" ]]; then
  echo "REPO_DIR is not set. Aborting."; exit 1
elif [[ "${MACTAHOE_SOURCE[@]}" =~ "lib-core.sh" ]]; then
  echo "lib-core.sh is already loaded. Aborting."; exit 1
fi
...
if [[ -d "${MACTAHOE_TMP_DIR}" ]]; then
  ...
  if [[ -d "${MACTAHOE_TMP_DIR}" ]]; then
    prompt -e "MacTahoe is already running (locked by '$(ls -ld "${MACTAHOE_TMP_DIR}" | awk '{print $3}')')."
  fi
fi
...
signal_error() {
  ...
  if [[ "$(grep -ril "Release" "${MACTAHOE_TMP_DIR}/error_log.txt")" == "${MACTAHOE_TMP_DIR}/error_log.txt" ]]; then
    prompt -i "Install the required dependencies for your distro (sassc, libglib2.0-dev, libxml2-utils).\n"
  fi

  prompt -i "Report this error at:\n  https://github.com/eprahemi/Fedora-MacTahoe-Eprahemi/issues\n"
...
dialogify() {
  if [[ "${silent_mode}" == "true" ]]; then
    prompt -w "Silent mode is active — dialog cannot be shown."
    return 0
  fi
...
check_param() {
  ...
  if [[ "${has_set["${global_param}"]}" == "true" ]]; then
    ...
    case "${must_not_ambigous}" in
      must)
        prompt -e "Ambiguous option '${display_param}' — choose one only."; has_any_error="true" ;;
      optional)
        prompt -w "Ambiguous option '${display_param}' — a chooser will be shown if possible." ;;
    esac
  fi
  ...
  if [[ "${value}" == "" || "${value}" == "-"*  ]]; then
    ...
    case "${must_have_value}" in
      must)
        prompt -e "'${display_param}' cannot be empty."; has_any_error="true" ;;
      optional)
        prompt -w "'${display_param}' is empty — a chooser will be shown if possible." ;;
    esac
    ...
    case "${global_param}" in
      -d)
        if [[ "$(readlink -m ${value})" =~ "${REPO_DIR}" ]]; then
          prompt -e "'${display_param}': cannot install in the source directory."
          has_any_error="true"
        elif [[ ! -w "${value}" && ! -w "$(dirname ${value})" ]]; then
          prompt -e "'${display_param}': permission denied — cannot write to that directory."
          has_any_error="true"
        else
          if [[ ! -d "${value}" ]]; then
            prompt -i "Destination directory does not exist. Creating it..."; echo
            mkdir -p "${value}"
          fi
      ...
      -b)
        if [[ "${value}" == "blank" || "${value}" == "default" ]]; then
          ...
          prompt -e "'${display_param}': image file not found or unreadable."
          has_any_error="true"
        elif file "${value}" | grep -qE "image|bitmap"; then
          background="${value}"
        else
          prompt -e "'${display_param}': not a valid image file."
          has_any_error="true"
        fi
    ...
    if [[ "${variant_found}" == "false" && "${variant_found}" != "skip" ]]; then
      case "${value_must_found}" in
        must)
          prompt -e "Unrecognized '${display_param}' variant: '${value}'."; has_any_error="true" ;;
        optional)
          prompt -w "'${display_param}' variant '${value}' is not recognized — a chooser will be shown if possible."
          need_dialog["${global_param}"]="true" ;;
      esac
    elif [[ "${allow_all_choice}" == "false" && "${value}" == "all" ]]; then
      prompt -e "Cannot use 'all' for '${display_param}'."; has_any_error="true"
    fi
...
remind_relative_path() {
  [[ "${2}" =~ "~" ]] && prompt -w "'${1}': note that '~/path' and '~ /path' are different."
}
...
sudo() {
  ...
  prompt -w "Running '$(echo "${@}" | cut -c -35 )...' as root.\n"

  if ! ${SUDO_BIN} -n true &> /dev/null; then
    echo -e "\n ${c_magenta} Authentication required${c_default} ${c_green}(enter your password):${c_default} \n"
  fi
...
udo() {
  ...
  if ! ${SUDO_BIN} -u "${MY_USERNAME}" -n true &> /dev/null; then
    prompt -w "Running '$(echo "${@}" | cut -c -35 )...' as user."
    echo -e "${c_magenta} Authentication required${c_default} ${c_green}(enter your password):${c_default}"
  fi
...
full_sudo() {
  if [[ ! -w "/root" ]]; then
    prompt -e "'${1}' requires root privileges. Run '${0}' as root."
    has_any_error="true"
  fi
}
...
usage() {
  prompt -e "Usage information is not available."; exit 1
}
...
finalize_argument_parsing() {
  if [[ "${need_help}" == "true" ]]; then
    ...
    echo; prompt -i "Run '$0 --help' for available options."; echo; exit 1
  fi
}
